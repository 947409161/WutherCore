...
自由订阅支持 WutherCore 原生 YAML/JSON、节点数组、命名节点映射、单节点文档和 URI，可通过 `type` 指定 Young 等协议，也可按字段自动探测；同时覆盖 Mihomo v1.19.29 的全部 26 类节点。订阅源、Age、字段和协议范围见 [自由订阅指南](docs/FEEDS.md)。
