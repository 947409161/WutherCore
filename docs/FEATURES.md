...
| 外部规则集 | 已实现 | Mihomo YAML/文本/MRS v1、sing-box JSON/SRS v1-v5、内联 Payload、RRS v2 |
| 路由匹配 | 已实现 | 域名、域名正则、源/目的 IP、源/目的端口、进程名、进程路径、规则集与嗅探信息 |
...
- 配置文件中的手动节点；
- 订阅中的 URI；
- WutherCore 原生 `nodes/outbounds` 文档与紧凑 `type` 节点；
- Mihomo/Clash 风格节点；
- 配置迁移生成的节点；
- 运行时订阅更新。
...
规则集运行时支持 Mihomo YAML/文本/MRS v1、sing-box JSON/SRS v1-v5 和 WutherCore RRS v2。Classical YAML/文本完整支持 `SRC-IP-CIDR`、`SRC-PORT`、`DOMAIN-REGEX` 和 `PROCESS-PATH`。MRS v1 保持上游定义的 domain/ipcidr behavior，不把 classical 规则伪装成非标准 MRS。二进制输入会先经过有界解压与结构校验，再编译为与文本规则共用的 matcher。
